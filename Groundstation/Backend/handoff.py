...
def getData():
    ...
    while True:
        ...
        if currdroneindex is None:
            new_drone = DroneData()
            new_drone.drone["sysid"] = currsysid
        ...
        if msg.get_type() == "GLOBAL_POSITION_INT":
            try:
                drone.drone["alt"] = msg.relative_alt / 1000 * 3.28084  # mm to feet
...
def doCommand():
    ...
    if command == "setradiopos":
        ...
        for drone in dronedata:
            drone.DFdata["measurements"].clear()
        cmdnum = 33337
        param2 = 1
...
if __name__ == "__main__":
    ...
    t1 = threading.Thread(target=getData, daemon=True)
    t2 = threading.Thread(target=sendheartbeat, daemon=True)
    t3 = threading.Thread(target=connectcountdown, daemon=True)

    t1.start()
    t2.start()
    t3.start()

    # Run Flask in main thread (blocking call)
